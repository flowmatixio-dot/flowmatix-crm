...
export default function App() {
  ...
  const [mfaLoginToken, setMfaLoginToken] = useState(null);
  ...
  const handleLogin=async()=>{setLoginErr("");setAuthLoading(true);
    ...
    try{const res=await fmApi.login(loginEmail,loginPass);if(res?.user){const u=res.user;const isOp=u.role==="platform_owner"||u.role==="admin";setUser({email:u.email,role:"admin",clinicId:isOp?null:"c1",name:u.name||u.email.split("@")[0],apiUser:true,apiRole:u.role,orgId:u.organizationId});if(isOp)setView("operator");setLang(loginLang);setAuthLoading(false);setAuditLog(prev=>[{id:genId(),time:new Date().toISOString(),user:u.name||u.email,role:u.role,clinicId:"admin",action:"login",target:"CRM",details:`API login from ${navigator.userAgent?.substring(0,50)}`},...prev]);return;}if(res?.requiresMfaSetup&&res?.setupToken){setMfaSetupToken(res.setupToken);setMfaSetupCode('');setMfaSetupQr('');setMfaSetupSecret('');setAuthLoading(false);fmApi.enrollWithToken(res.setupToken).then(r=>{const uri=r.otpauthUri||'';const qr=uri?`https://api.qrserver.com/v1/create-qr-code/?size=180x180&data=${encodeURIComponent(uri)}`:'';setMfaSetupQr(qr);setMfaSetupSecret(r.secret||'');}).catch(()=>{});setLoginMode('mfaSetup');return;}if(res?.requiresMfa&&res?.mfaToken){setMfaLoginToken(res.mfaToken);setMfaLoginCode('');setLoginErr('');setAuthLoading(false);setLoginMode('mfa');return;}}catch(e){/* API login failed — continue to Supabase/demo */}
  ...
  if(!user)return(
    <div style={{minHeight:"100vh",background:AUTH_BG,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",fontFamily:"'Plus Jakarta Sans',system-ui,sans-serif",position:"relative"}}>
      ...
      <div style={{width:420,padding:"36px 40px",borderRadius:20,background:"#162032",border:"1px solid rgba(255,255,255,0.08)",position:"relative",zIndex:1,boxShadow:"0 8px 24px rgba(0,0,0,0.3)"}}>
        ...
        {/* ═══ MODE: MFA VERIFY — enter 6-digit TOTP code ═══ */}
        {loginMode==="mfa"&&<div style={{textAlign:"center"}}>
          <div style={{fontSize:32,marginBottom:8}}>🔐</div>
          <div style={{fontWeight:800,fontSize:20,marginBottom:4,color:"#fff"}}>2FA-Code eingeben</div>
          <div style={{fontSize:13,color:"rgba(167,177,195,0.6)",marginBottom:20}}>Code aus deiner Authenticator-App</div>
          <input type="text" inputMode="numeric" maxLength={6} value={mfaLoginCode} onChange={e=>setMfaLoginCode(e.target.value.replace(/\D/g,""))} onKeyDown={async e=>{if(e.key==="Enter"&&mfaLoginCode.length===6){setMfaLoginLoading(true);setLoginErr('');try{const r=await fmApi.verifyMfaLogin(mfaLoginToken,mfaLoginCode);if(r?.accessToken){fmApi.setTokens(r.accessToken,r.refreshToken);const me=await fmApi.getMe();if(me){setUser({email:me.email,role:"admin",name:me.name||me.email.split("@")[0],apiUser:true,apiRole:me.role,orgId:me.organizationId});setView("operator");setAuthLoading(false);setLoginMode("password");return;}}}catch(err){setLoginErr(err.message||"Ungültiger Code");}setMfaLoginLoading(false);}}} placeholder="000000" autoFocus style={{width:"100%",padding:"14px",borderRadius:12,background:"rgba(255,255,255,0.05)",border:"1px solid rgba(76,201,255,0.2)",color:"#fff",fontFamily:"inherit",fontSize:22,fontWeight:700,outline:"none",boxSizing:"border-box",textAlign:"center",letterSpacing:8,marginBottom:12}}/>
          ...
          <button onClick={async()=>{if(mfaLoginCode.length!==6)return;setMfaLoginLoading(true);setLoginErr('');try{const r=await fmApi.verifyMfaLogin(mfaLoginToken,mfaLoginCode);if(r?.accessToken){fmApi.setTokens(r.accessToken,r.refreshToken);const me=await fmApi.getMe();if(me){setUser({email:me.email,role:"admin",name:me.name||me.email.split("@")[0],apiUser:true,apiRole:me.role,orgId:me.organizationId});setView("operator");setAuthLoading(false);setLoginMode("password");return;}}}catch(err){setLoginErr(err.message||"Ungültiger Code");}setMfaLoginLoading(false);}} disabled={mfaLoginLoading||mfaLoginCode.length!==6} style={{width:"100%",padding:14,borderRadius:12,background:mfaLoginCode.length===6?"linear-gradient(135deg,#00B4D8,#0096c7)":"rgba(255,255,255,0.04)",border:"none",color:mfaLoginCode.length===6?"#fff":"rgba(167,177,195,0.4)",fontWeight:700,fontSize:15,cursor:mfaLoginCode.length===6?"pointer":"default",fontFamily:"inherit"}}>{mfaLoginLoading?"…":"Einloggen →"}</button>
          <button onClick={()=>{setLoginMode("password");setLoginErr("");setMfaLoginToken(null);setMfaLoginCode('');}} style={{background:"none",border:"none",color:"rgba(167,177,195,0.4)",cursor:"pointer",fontSize:12,fontFamily:"inherit",marginTop:12}}>← Zurück</button>
        </div>}
  ...
  return(
    ...
    <div style={{display:"flex",height:"calc(100vh / 1.04)",background:"var(--bg-app)",fontFamily:"'Plus Jakarta Sans',system-ui,sans-serif",color:"var(--text-primary)",overflow:"hidden",transition:"background .25s ease, color .25s ease"}}>
      ...
      <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
        {/* TOP BAR */}
        <div className="fm-topbar" style={{height:56,minHeight:56,borderBottom:"1px solid var(--border-default)",background:"var(--bg-topbar)",display:"flex",alignItems:"center",padding:"0 28px",gap:14,flexShrink:0,transition:"background .25s ease, border-color .25s ease"}}>
