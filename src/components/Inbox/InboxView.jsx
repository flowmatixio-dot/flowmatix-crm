...
function CaseOverviewPanel({ chat, lead, t, onClose, clinic }) {
  ...
  useEffect(() => {
    ...
    fmApi.translateBatch(items, lang).then(res => {
      if (res?.translations) setTrFields(res.translations);
    }).catch(() => {});
  }, [intakeKey]);
  const intake = { ...rawIntake };
  if (Object.keys(trFields).length) {
    Object.entries(trFields).forEach(([k, v]) => { if (v && rawIntake[k]) intake[k] = v; });
  }
...
export default function InboxView() {
  ...
  useEffect(() => {
    ...
    if (!pid || selChat.is_demo) return;
    const refreshPatient = () => {
      fmApi.apiFetch('/api/v1/crm/patients/' + pid).then(function(res) {
        const p = res && (res.patient || res);
        if (p && p.id) {
          var store = usePatientStore.getState();
    ...
    refreshPatient();
    var iv2 = setInterval(refreshPatient, 10000);
            ...
            <>
              ...
              {isReviewWait && (
                <div style={{padding:"10px 22px",background:"rgba(255,138,42,0.06)",borderBottom:"1px solid rgba(255,138,42,0.15)",display:"flex",alignItems:"center",justifyContent:"space-between",flexShrink:0}}>
                  ...
                  <button onClick={() => { window.dispatchEvent(new CustomEvent('fm-open-review')); }} style={{padding:"5px 14px",borderRadius:8,background:"rgba(255,138,42,0.1)",border:"1px solid rgba(255,138,42,0.25)",color:"#ff8a2a",fontSize:11,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>
                ...
                if (isReviewWait) return (
                  <div style={{padding:"14px 22px",borderTop:"1px solid var(--border-default)",flexShrink:0}}>
                    <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"12px 18px",borderRadius:14,background:"rgba(255,138,42,0.04)",border:"1px solid rgba(255,138,42,0.12)"}}>
                      ...
                      <button onClick={() => { window.dispatchEvent(new CustomEvent('fm-open-review')); }} style={{padding:"6px 16px",borderRadius:8,background:"rgba(255,138,42,0.1)",border:"1px solid rgba(255,138,42,0.25)",color:"#ff8a2a",fontSize:11,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>
