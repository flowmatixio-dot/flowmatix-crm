...
export default function AIControlView() {
  ...
  const saveAi = () => { setClinics(cs => cs.map(c => c.id === activeClinicId ? {...c, aiConfig: {...aiConfigData}} : c)); showT("AI config saved"); };

  return <div style={{padding:"28px 32px",maxWidth:800}}>
    <h1 style={{fontSize:24,fontWeight:800,margin:"0 0 6px",letterSpacing:"-0.02em"}}>AI Control</h1>
    <p style={{fontSize:14,color:"rgba(167,177,195,0.5)",margin:"0 0 28px"}}>Configure how your AI assistant behaves.</p>
    {/* AI Metrics */}
    ...
    <button onClick={saveAi} style={{padding:"14px 32px",borderRadius:14,background:"linear-gradient(135deg,#ff8a2a,#ff6b00)",border:"none",color:"#fff",fontWeight:700,fontSize:15,cursor:"pointer",fontFamily:"inherit"}}>Save AI Config</button>
  </div>;
}
