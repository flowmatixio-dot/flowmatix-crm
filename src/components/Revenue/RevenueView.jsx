  ...
  return <div style={{ padding: 28, }}>
    ...
    {tab === "overview" && <>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(5,1fr)", gap: 14, marginBottom: 28 }}>
        <div style={{ padding: 18, borderRadius: 16, background: "rgba(16,185,129,0.04)", border: "1px solid rgba(16,185,129,0.15)" }}>
          ...
          <div style={{ fontSize: 11, color: "rgba(167,177,195,0.4)", marginTop: 4 }}>{kpis.paidInvCount} {t("completed")||"abgeschlossen"}</div>
        </div>
        <div style={{ padding: 18, borderRadius: 16, background: "rgba(76,201,255,0.04)", border: "1px solid rgba(76,201,255,0.15)" }}>
          ...
          <div style={{ fontSize: 11, color: "rgba(167,177,195,0.4)", marginTop: 4 }}>{kpis.depositsReceived} {t("deposits_received")||"erhalten"}</div>
        </div>
        <div style={{ padding: 18, borderRadius: 16, background: "rgba(255,138,42,0.04)", border: "1px solid rgba(255,138,42,0.15)" }}>
          ...
          <div style={{ fontSize: 11, color: "rgba(167,177,195,0.4)", marginTop: 4 }}>{kpis.patientsPending} {t("patients_pending")||"ausstehend"}</div>
        </div>
        <div style={{ padding: 18, borderRadius: 16, background: "rgba(251,191,36,0.04)", border: "1px solid rgba(251,191,36,0.15)" }}>
          ...
          <div style={{ fontSize: 11, color: "rgba(167,177,195,0.4)", marginTop: 4 }}>{kpis.completedTreatments} {t("treatments_done")||"Behandlungen"}</div>
        </div>
        <div style={{ padding: 18, borderRadius: 16, background: "rgba(167,107,255,0.04)", border: "1px solid rgba(167,107,255,0.15)" }}>
          ...
          <div style={{ fontSize: 11, color: "rgba(167,177,195,0.4)", marginTop: 4 }}>{kpis.completedTreatments}/{myLeads.length} {t("leads")||"Leads"}</div>
        </div>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gap: 16, marginBottom: 28 }}>
        ...
        <div style={{ padding: 20, borderRadius: 16, background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.06)" }}>
          <div style={{ fontSize: 11, fontWeight: 700, color: "rgba(167,177,195,0.4)", textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: 16 }}>{t("monthly_revenue_6mo")||"REVENUE · LAST 6 MONTHS"}</div>
        ...
        {/* Payment Method Breakdown */}
        <div style={{ padding: 20, borderRadius: 16, background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.06)" }}>
          <div style={{ fontSize: 11, fontWeight: 700, color: "rgba(167,177,195,0.4)", textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: 16 }}>{t("payment_methods")||"PAYMENT METHODS"}</div>
          {paymentMethods.length === 0 && <div style={{ textAlign: "center", padding: 20, color: "rgba(167,177,195,0.3)", fontSize: 13 }}>{t("no_payments_yet")}</div>}
      ...
      {kpis.pending > 0 && <div style={{ padding: 16, borderRadius: 14, background: "rgba(255,138,42,0.04)", border: "1px solid rgba(255,138,42,0.15)", marginBottom: 28, display: "flex", alignItems: "center", gap: 12 }}>
        ...
        <div>
          ...
          <div style={{ fontSize: 12, color: "rgba(167,177,195,0.5)", marginTop: 2 }}>€{kpis.pending.toLocaleString()} {t("outstanding_amount_desc")||"offen"}</div>
        </div>
      ...
      <div style={{ fontSize: 11, fontWeight: 700, color: "rgba(167,177,195,0.4)", textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: 12 }}>{t("rev_patient_payment_status")||"Patient Payment Status"}</div>
      ...
      <Section title={t("recent_payments")||"RECENT PAYMENTS"}>
        ...
        {clinicInvoices.filter(i => i.status === "paid").length === 0 && <div style={{ textAlign: "center", padding: 30, color: "rgba(167,177,195,0.4)" }}>{t("no_payments_received_yet")}</div>}
      </Section>
    ...
    {tab === "invoices" && <>
      ...
      {newInvOpen && <div style={{ marginBottom: 20, padding: 20, borderRadius: 14, background: "rgba(16,185,129,0.04)", border: "1px solid rgba(16,185,129,0.12)" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 12 }}>
          <div>
            <div style={{ fontSize: 11, fontWeight: 700, color: "rgba(167,177,195,0.5)", marginBottom: 4 }}>{t("patient")}</div>
          ...
          <div>
            <div style={{ fontSize: 11, fontWeight: 700, color: "rgba(167,177,195,0.5)", marginBottom: 4 }}>{t("net_amount_eur")}</div>
            <input value={newInvNet} onChange={e => setNewInvNet(e.target.value)} placeholder="2400" type="number" style={inp} />
          </div>
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gap: 12, marginBottom: 12 }}>
          <div>
            <div style={{ fontSize: 11, fontWeight: 700, color: "rgba(167,177,195,0.5)", marginBottom: 4 }}>{t("items_description")}</div>
          ...
          <div>
            <div style={{ fontSize: 11, fontWeight: 700, color: "rgba(167,177,195,0.5)", marginBottom: 4 }}>{t("vat_percent")}</div>
      ...
      {filtered.length === 0 && <div style={{ textAlign: "center", padding: 30, color: "rgba(167,177,195,0.4)" }}>{t("no_invoices_match_filter")}</div>}
      {filtered.map(inv => {
      ...
      {clinicInvoices.length > 0 && <div style={{ marginTop: 16, padding: 14, borderRadius: 12, background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.06)", display: "flex", justifyContent: "space-between", fontSize: 13 }}>
        <span style={{ color: "rgba(167,177,195,0.5)" }}>{clinicInvoices.length} {clinicInvoices.length !== 1 ? t("invoices_total") : t("invoice_total")}</span>
    ...
    {tab === "exports" && <>
      ...
      {isAuthenticated() && <div style={{ padding: 24, borderRadius: 16, background: "rgba(16,185,129,0.04)", border: "1px solid rgba(16,185,129,0.15)", marginBottom: 20, position: "relative", opacity: isCorePlan ? 0.45 : 1, pointerEvents: isCorePlan ? "none" : "auto" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16 }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 12, color: "rgba(167,177,195,0.5)" }}>{t("google_sheets_export_desc")}</div>
          </div>
        </div>
        ...
        <div style={{ fontSize: 11, color: "rgba(167,177,195,0.35)", marginTop: 10 }}>{t("requires_google_sheets_connection")}</div>
      </div>}
        ...
        <div style={{ padding: 24, borderRadius: 16, background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.06)" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16 }}>
            ...
            <div>
              ...
              <div style={{ fontSize: 12, color: "rgba(167,177,195,0.5)" }}>{t("datev_export_desc")}</div>
            </div>
          </div>
          <div style={{ marginBottom: 12 }}>
            <div style={{ fontSize: 11, fontWeight: 700, color: "rgba(167,177,195,0.5)", marginBottom: 4 }}>{t("month")}</div>
        ...
        <div style={{ padding: 24, borderRadius: 16, background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.06)" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16 }}>
            ...
            <div>
              ...
              <div style={{ fontSize: 12, color: "rgba(167,177,195,0.5)" }}>{t("csv_export_desc")}</div>
            </div>
          </div>
          <div style={{ marginBottom: 12 }}>
            <div style={{ fontSize: 11, fontWeight: 700, color: "rgba(167,177,195,0.5)", marginBottom: 4 }}>{t("month")}</div>
      ...
      <Section title={t("invoice_pdfs")}>
        <div style={{ fontSize: 13, color: "rgba(167,177,195,0.5)", marginBottom: 14 }}>{t("generate_pdf_invoices_desc")}</div>
        {clinicInvoices.length === 0 && <div style={{ textAlign: "center", padding: 20, color: "rgba(167,177,195,0.4)" }}>{t("no_invoices_to_export")}</div>}
  ...
  return (
    <>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 28 }}>
        {/* Per Doctor */}
        <div style={cardStyle}>
          <div style={{ fontSize: 11, fontWeight: 700, color: "rgba(167,177,195,0.4)", textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: 16 }}>{t("revenue_per_doctor")||"Revenue per Doctor"}</div>
          {doctorRevenue.length === 0 && <div style={{ textAlign: "center", padding: 20, color: "rgba(167,177,195,0.3)", fontSize: 12 }}>{t("no_data_yet")||"No data yet"}</div>}
          {doctorRevenue.map(([doc, data]) => (
            <div key={doc} style={{ marginBottom: 10 }}>
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, marginBottom: 4 }}>
                <span style={{ fontWeight: 600, color: "rgba(232,238,252,0.8)" }}>{doc}</span>
                <span style={{ fontWeight: 700, color: "#10b981" }}>{fmtEur(data.revenue)} <span style={{ color: "rgba(167,177,195,0.35)", fontWeight: 500 }}>({data.ops} OPs)</span></span>
              </div>
              <div style={{ height: 5, borderRadius: 3, background: "rgba(255,255,255,0.06)" }}>
        ...
        {/* Per Treatment */}
        <div style={cardStyle}>
          <div style={{ fontSize: 11, fontWeight: 700, color: "rgba(167,177,195,0.4)", textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: 16 }}>{t("revenue_per_treatment")||"Revenue per Treatment"}</div>
          {treatmentRevenue.length === 0 && <div style={{ textAlign: "center", padding: 20, color: "rgba(167,177,195,0.3)", fontSize: 12 }}>{t("no_data_yet")||"No data yet"}</div>}
              ...
              <div key={treat} style={{ marginBottom: 10 }}>
                <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, marginBottom: 4 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                    ...
                    <span style={{ fontWeight: 600, color: "rgba(232,238,252,0.8)" }}>{treat}</span>
                  </div>
                  <span style={{ fontWeight: 700, color }}>{fmtEur(data.revenue)} <span style={{ color: "rgba(167,177,195,0.35)", fontWeight: 500 }}>({data.count}x)</span></span>
                </div>
                <div style={{ height: 5, borderRadius: 3, background: "rgba(255,255,255,0.06)" }}>
          ...
          <div style={cardStyle}>
            {countryRevenue.map(([country, data]) => (
              <div key={country} style={{ marginBottom: 10 }}>
                <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, marginBottom: 4 }}>
                  <span style={{ fontWeight: 600, color: "rgba(232,238,252,0.8)" }}>{country} <span style={{ color: "rgba(167,177,195,0.35)", fontWeight: 500 }}>({data.count} Leads)</span></span>
