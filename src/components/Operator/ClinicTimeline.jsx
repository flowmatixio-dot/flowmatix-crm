...
const EVENT_META = {
  ...
  bot_activated:        { icon: "\u{1F916}", color: "#10b981", label: "Bot Activated" },
  ...
  automation_run:       { icon: "⚡", color: "#eab308", label: "Automation Run" },
  invoice_created:      { icon: "\u{1F4B0}", color: "#10b981", label: "Invoice Created" },
  message_sent:         { icon: "\u{1F4E4}", color: "#6b7280", label: "Message Sent" },
  provisioning:         { icon: "\u{1F680}", color: "#00B4D8", label: "Provisioning" },
  subscription_change:  { icon: "\u{1F4B3}", color: "#a78bfa", label: "Subscription Change" },
  appointment_cancelled_by_patient: { icon: "\u{274C}", color: "#ef4444", label: "Termin storniert" },
  appointment_booked_by_bot: { icon: "\u{1F4C5}", color: "#10b981", label: "Termin gebucht" },
...
export default function ClinicTimeline({ orgId }) {
  ...
  return (
    <div style={{ position: "relative", paddingLeft: 32 }}>
      {/* Vertical line */}
      <div style={{ position: "absolute", left: 13, top: 8, bottom: 8, width: 2, background: "rgba(255,255,255,0.06)", borderRadius: 1 }} />
        ...
        return (
          <div key={evt.id || i} style={{ position: "relative", marginBottom: 16, paddingLeft: 16 }}>
            ...
            <div style={{
              ...
              background: "#23234a", border: "1px solid rgba(255,255,255,0.06)",
            }}>
