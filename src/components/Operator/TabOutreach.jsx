...
export default function TabOutreach({ d, load }) {
  ...
  const refresh = () => {
    load('outreach', () => api.getOutreachLeads(filter && !['replied','dnc'].includes(filter) ? { status: filter } : {}));
    load('outreachStats', api.getOutreachStats);
  };

  const doSearch = () => {
    load('outreach', () => api.getOutreachLeads(search ? { search } : {}));
  };
  ...
  const todayRemaining = stats.today_remaining ?? 30;
    ...
    <>
      ...
      <div style={{ ...S.card, padding: 16, marginBottom: 12 }}>
        ...
        <div style={{ fontSize: 11, color: '#8888aa', marginTop: 4 }}>
          {todaySent >= 30 ? 'Tageslimit erreicht' : `Noch ${todayRemaining} ausstehend — nächster Versand heute 05:15 CEST`}
        </div>
      </div>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 12, flexWrap: 'wrap' }}>
        ...
        <div onClick={() => setFilter(filter === 'replied' ? null : 'replied')} style={{
          ...
          <span style={{ fontSize: 17, fontWeight: 800, color: S.green }}>
            {leads.filter(l => l.replied_at).length}
          </span>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
        ...
        {(search || filter) && <Btn onClick={() => { setSearch(''); setFilter(null); load('outreach', api.getOutreachLeads); load('outreachStats', api.getOutreachStats); }}>Reset</Btn>}
      </div>
      ...
      {!d.outreach ? (
        ...
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
            <thead>
              <tr>
                {['Klinik', 'E-Mail', 'Sprache', 'Status', 'Mail #', 'Zuletzt gesendet', 'Nächster Versand', 'Aktionen'].map(h => (
                ...
                <>
                  <tr key={lead.id} style={{ opacity: lead.do_not_contact ? 0.45 : 1 }}>
                    <td style={{ padding: '10px 12px', borderBottom: '1px solid #1e1e3e', color: '#fff', fontWeight: 600 }}>
                      ...
                      {lead.clinic_name}
                      {lead.homepage && (
                        <a href={lead.homepage.startsWith('http') ? lead.homepage : `https://${lead.homepage}`} target="_blank" rel="noopener"
                          style={{ marginLeft: 6, fontSize: 10, color: S.accent, textDecoration: 'none' }}>↗</a>
                      )}
                    </td>
                    ...
                    <td style={{ padding: '10px 12px', borderBottom: '1px solid #1e1e3e', color: '#8888aa', whiteSpace: 'nowrap' }}>
                      {lead.last_sent_at ? fmtDate(lead.last_sent_at) : '—'}
                    </td>
                    <td style={{ padding: '10px 12px', borderBottom: '1px solid #1e1e3e', color: '#8888aa', whiteSpace: 'nowrap' }}>
                      {lead.next_send_at
                        ? <span style={{ color: new Date(lead.next_send_at) < new Date() ? S.yellow : '#8888aa' }}>{fmtDate(lead.next_send_at)}</span>
                        : '—'}
                    </td>
                    <td style={{ padding: '10px 12px', borderBottom: '1px solid #1e1e3e' }}>
                      <div style={{ display: 'flex', gap: 6 }}>
                        <Btn small onClick={() => loadLogs(lead.id)}>
                          {logs[lead.id] ? 'Logs ▲' : 'Logs ▼'}
                        </Btn>
                        <Btn small danger={!lead.do_not_contact} onClick={() => toggleDnc(lead)}>
                          {lead.do_not_contact ? 'Entsperren' : 'Blockieren'}
                        </Btn>
                      </div>
                    </td>
                  </tr>
                    ...
                    <tr key={`logs-${lead.id}`}>
                      <td colSpan={8} style={{ padding: '0 12px 12px 32px', borderBottom: '1px solid #1e1e3e', background: '#1a1a2e' }}>
                        {logs[lead.id].length === 0 ? (
                          <span style={{ fontSize: 12, color: '#555' }}>Keine Logs vorhanden</span>
                        ) : (
