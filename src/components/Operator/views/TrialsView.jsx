...
const ACTION_CTA = {
  START_SETUP: 'Start Setup', WAIT_FOR_NUMBER: 'Waiting...', CONNECT_WHATSAPP: 'Connect WA',
  VERIFY_OTP: 'Verify OTP', FIX_ERROR: 'Fix Error', ACTIVATE: 'Activate', NONE: null,
};
            ...
            {col.items.map(c => {
              ...
              return (
                <div key={c.id} onClick={() => navigateTo?.('clinics', c)}
                  style={{ background: 'var(--bg-card)', borderRadius: 10, padding: '14px 16px', cursor: 'pointer', transition: 'all 0.15s', border: isCritical ? '1px solid rgba(239,68,68,0.3)' : '1px solid rgba(255,255,255,0.04)', boxShadow: isCritical ? '0 0 12px rgba(239,68,68,0.08)' : 'none' }}
                  ...
                  <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--text-primary)', marginBottom: 2 }}>{safeStr(c.name)}</div>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
                    ...
                    {daysLeft !== null && <span style={{ fontSize: 11, fontWeight: 700, color: isCritical ? '#ef4444' : daysLeft < 5 ? '#f97316' : 'var(--text-muted)' }}>{daysLeft}d left</span>}
                  </div>
                  ...
                  {cta && (
                    <button onClick={e => { e.stopPropagation(); navigateTo?.('clinics', c); }}
                      style={{ width: '100%', padding: '7px 0', borderRadius: 6, border: 'none', background: isCritical ? '#ef4444' : `${col.color}20`, color: isCritical ? '#fff' : col.color, fontSize: 11, fontWeight: 700, cursor: 'pointer' }}>
                      {cta}
                    </button>
                  )}
                </div>
