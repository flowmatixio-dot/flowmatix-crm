...
export default function AutomationsView() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      setQueueStats(qs); setN8n(n); setJobs(Array.isArray(j?.jobs) ? j.jobs : []); setLoading(false);
    });
  }, []);
  ...
  const queueColumns = [
    ...
    { key: 'failed', label: 'Failed', render: v => { const n = safeNum(v); return <span style={{ color: n > 0 ? '#ef4444' : 'var(--text-muted)', fontWeight: n > 0 ? 700 : 400 }}>{n}</span>; }},
    { key: 'dead_letter', label: 'Dead Letter', render: v => { const n = safeNum(v); return <span style={{ color: n > 0 ? '#ef4444' : 'var(--text-muted)' }}>{n}</span>; }},
  ];

  const jobColumns = [
    { key: 'queue_name', label: 'Queue', render: v => <span style={{ fontSize: 12 }}>{safeStr(v)}</span> },
    { key: 'job_name', label: 'Job', render: v => safeStr(v) },
    { key: 'error_message', label: 'Error', render: v => <span style={{ fontSize: 11, color: '#ef4444', maxWidth: 300, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', display: 'inline-block' }}>{safeStr(v, 'Unknown error')}</span> },
  ...
  return (
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16, marginBottom: 28 }}>
        ...
        <StatCard label="n8n Workflows" value={workflows.length} color="purple" sub={`${workflows.filter(w => w.active === true).length} active`} />
      </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280, 1fr))', gap: 12 }}>
            {workflows.map((w, i) => (
              <div key={w.id || i} style={{ background: 'var(--bg-card)', borderRadius: 10, padding: '14px 18px', borderLeft: `3px solid ${w.active === true ? '#10b981' : '#6b7280'}` }}>
                ...
                <div style={{ fontSize: 11, color: w.active === true ? '#10b981' : '#6b7280', marginTop: 4 }}>{w.active === true ? '● Active' : '○ Inactive'}</div>
              </div>
            ))}
          </div>
        </div>
