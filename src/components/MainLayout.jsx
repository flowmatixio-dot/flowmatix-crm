    ...
    <div style={{ display: "flex", height: "calc(100vh / 1.04)", background: "var(--bg-app)", fontFamily: "'Plus Jakarta Sans',system-ui,sans-serif", color: "var(--text-primary)", overflow: "hidden", transition: "background .25s ease, color .25s ease", paddingTop: showVerbisBanner ? 44 : 0, ...(ctx.workspaceState === 'trial_expired' || ctx.showPlanPicker ? { filter: "blur(3px)", pointerEvents: "none" } : {}) }}>
      ...
      <div style={{ width: sidebar ? 260 : 68, minWidth: sidebar ? 260 : 68, background: "#131c2e", borderRight: "1px solid rgba(255,255,255,0.06)", display: "flex", flexDirection: "column", transition: "all .3s cubic-bezier(.4,0,.2,1)", overflow: "visible", position: "relative" }}>
        ...
        <div style={{ padding: sidebar ? "20px 18px 16px" : "20px 14px 16px", borderBottom: "1px solid rgba(255,255,255,0.05)", overflow: "hidden" }}>
          ...
          {sidebar && isAdmin && !isOperator && clinic?.plan === 'enterprise' && (clinic?.aiConfig?.locations?.length > 0) && (
